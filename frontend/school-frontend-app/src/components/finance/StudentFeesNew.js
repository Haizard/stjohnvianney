...
const StudentFeesNew = () => {
  ...
  const handleAssignFeeStructure = async () => {
    if (!selectedStudent || !selectedFeeStructure) {
      setError('Please select both a student and a fee structure.');
      return;
    }
    ...
    try {
      ...
      const response = await api.post('/api/finance/student-fees/assign', {
      ...
      setError(error.response?.data?.message || 'Failed to assign fee structure. Please try again later.');
